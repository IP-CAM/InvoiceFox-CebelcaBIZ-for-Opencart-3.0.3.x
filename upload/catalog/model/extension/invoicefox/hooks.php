...
class ModelExtensionInvoiceFoxHooks extends Model { 
  ...
  public function changeOrderStatusHook($status_id, $comment, $order_id) {
    ...
    $this->load->model('extension/invoicefox/order_status');
    
    $order_status_info = $this->model_extension_invoicefox_order_status->getOrderStatus($status_id);
	...
	$invoicefox_id = $this->model_extension_invoicefox_order->getInvoicefoxId($order_id); 
  ...
  private function makeInvoiceFromOrder($order_id) {
	...
	$products = $this->model_extension_invoicefox_order->getOrderProducts($order_id);
...
//	  $producttax = round($product['tax'] / $product['price'] * 100, $this->CONF['round_calculated_taxrate_to']);
	...
	$coupon = $this->findCoupon($this->model_extension_invoicefox_order->getOrderTotals($order_id));
	...
	if ($r2->isOk()) {    
	  $invA = $r2->getData();
	  $this->model_extension_invoicefox_order->insertInvoicefoxId($order_id,$invA[0]['id']);
